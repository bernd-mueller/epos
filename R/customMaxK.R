customMaxK <- function(lists, L, d, v, threshold=50) {
  ...
  res.j0.temp <- TopKLists::j0.multi(lists, d, v)
  ...
  if(sum(is.na(res.temp[,5]))<nrow(res.temp)){
    ...
    sp = rep(list(tli),L)
    resS = TopKLists::CEMC(input = tl, space = sp, k = maxK)
    temp = tapply(as.numeric(res.temp[, 5]), res.temp[, 1])
    ...
    temp2 = tapply(as.numeric(res.temp2[,5]), res.temp2[,1])
